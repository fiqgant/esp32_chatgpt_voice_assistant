...
void I2SAudioRecord(void *arg)
{
  ...
  xSemaphoreGive(i2sFinishedSemaphore); // After completing the I2SAudioRecord task, pass the semaphore
  vTaskDelete(NULL);
}
...
void uploadFile()
{
  ...
  HTTPClient client;
  client.setTimeout(20000); // Increase timeout to 20 seconds
  ...
  if (httpResponseCode > 0)
  {
    ...
    {
      Serial.println("Failed to upload file. Response code: " + String(httpResponseCode));
    }
  }
  else
  {
    ...
    Serial.println(httpResponseCode);
    Serial.println(client.errorToString(httpResponseCode)); // Print the error string
  }

  file.close();
  client.end();

  // Frees up I2S resources
